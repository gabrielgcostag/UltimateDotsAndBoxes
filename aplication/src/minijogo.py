...
class Minijogo:
    ...
    def drawVisualLine(self, canvas, line):
        # Unpack line details
        (start_dot_coords, end_dot_coords, direction) = line
        start_row, start_col = start_dot_coords
        end_row, end_col = end_dot_coords

        # Calculate canvas coordinates for the start and end dots
        start_x = (self.section_col * 200) + self.margin + self.spacing * start_col
        start_y = (self.section_row * 200) + self.margin + self.spacing * start_row

        end_x = (self.section_col * 200) + self.margin + self.spacing * end_col
        end_y = (self.section_row * 200) + self.margin + self.spacing * end_row
